...
contains
  ...
  subroutine topo_read(topo_file)
    ...
    case ('mola32')
      ...
      deallocate(tmp)
    case default
      call log_error('Unknown topo_type "' // trim(topo_type) // '"!')
    end select
  ...
  subroutine topo_regrid(block)
    ...
    associate (mesh     => block%mesh              , &
               ...
               landmask => block%static(ivector)%landmask, &
               gzs      => block%static(ivector)%gzs     , &
               zs_std   => block%static(ivector)%zs_std)
  ...
  subroutine topo_smooth(block)
    ...
    associate (mesh => block%mesh, state=> block%state(1,ivector),gzs => block%static(ivector)%gzs, landmask => block%static(ivector)%landmask)
