module operators_mod
  ...
  external :: slave_calc_div , slave_calc_div_south , slave_calc_div_north
...
contains
  ...
  subroutine calc_div(block, state)
    ...
    call Get_Start_Time(cal_time_start)
            ...
            vd%v0 = mesh%full_lon_ibeg
            vd%v1 = mesh%full_lon_iend
            vd%v2 = mesh%full_lat_ibeg
            vd%v3 = mesh%full_lat_iend
            vd%v4 = mesh%full_lev_ibeg
            vd%v5 = mesh%full_lev_iend
            vd%v6 = proc%lon_halo_width
            vd%v7 = 2  
            vd%v8 = proc%id
            ...
            vd%vb = loc(u)
            vd%vc = loc(v)
            vd%vd = loc(mesh%area_cell)
    

            

            if (mesh%has_north_pole()) then
              call athread_spawn(slave_calc_div_north, vd)
              call athread_join()
            else if (mesh%has_south_pole()) then
              call athread_spawn(slave_calc_div_south, vd)
              call athread_join()
            else
              call athread_spawn(slave_calc_div, vd)
              call athread_join()
            end if
    ...
    call Get_End_Time(cal_time_end)
    cal_time = cal_time + cal_time_end - cal_time_start
    div_time = div_time + cal_time_end - cal_time_start
    call Get_Start_Time(cal_time_start)
#endif
