...
contains
  ...
  subroutine vor_damp_run(block, dt, state)
    ...
    case (2)
      ...
      if (use_implicit_solver(mesh%half_lat_ibeg_no_pole) .eqv. .true. .or. &
          use_implicit_solver(mesh%half_lat_iend_no_pole) .eqv. .true.) call state%async(async_u)%wait() ! 如果是隐式耗散需要等待用新的u
#ifdef Detail_Time
      call Get_End_Time(tran_time_end)
      tran_time = tran_time + tran_time_end - tran_time_start
#endif
