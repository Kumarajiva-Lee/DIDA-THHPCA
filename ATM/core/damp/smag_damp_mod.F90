...
contains
  ...
  subroutine smag_damp_run(block, dt, tend, state)
    ...
    associate (mesh      => block%mesh     , &
      ...
      call Get_Start_Time(tran_time_start)
#endif
      call fill_halo_member(block, tension_h, full_lon=.true., full_lat=.true., full_lev=.true., async=state%async(async_tension_h))
#ifdef Detail_Time
      call Get_End_Time(tran_time_end)
      tran_time = tran_time + tran_time_end - tran_time_start
#endif
      ! call fill_halo_member(block, tension_h, full_lon=.true., full_lat=.true., full_lev=.true.)
      ...
      call Get_Start_Time(tran_time_start)
#endif
      call fill_halo_member(block, shear_h, full_lon=.false., full_lat=.false., full_lev=.true., async=state%async(async_shear_h))
      ! call fill_halo_member(block, shear_h, full_lon=.false., full_lat=.false., full_lev=.true.)
      
      call state%async(async_tension_h)%wait()
      call state%async(async_shear_h)%wait()      
#ifdef Detail_Time
      call Get_End_Time(tran_time_end)
      tran_time = tran_time + tran_time_end - tran_time_start
#endif
      ...
      call Get_Start_Time(tran_time_start)
#endif
      call fill_halo_member(block, kmh, full_lon=.true., full_lat=.true., full_lev=.true., async=state%async(async_kmh))
#ifdef Detail_Time
      call Get_End_Time(tran_time_end)
      tran_time = tran_time + tran_time_end - tran_time_start
#endif
      ! call fill_halo_member(block, kmh, full_lon=.true., full_lat=.true., full_lev=.true.)
      ...
      call Get_Start_Time(tran_time_start)
#endif
      call fill_halo_member(block, kmh_vtx, full_lon=.false., full_lat=.false., full_lev=.true., async=state%async(async_kmh_vtx))
#ifdef Detail_Time
      call Get_End_Time(tran_time_end)
      tran_time = tran_time + tran_time_end - tran_time_start
#endif
      ! call fill_halo_member(block, kmh_vtx, full_lon=.false., full_lat=.false., full_lev=.true.)
      ...
      call Get_Start_Time(tran_time_start)
#endif
      call fill_halo_member(block, kmh_lon, full_lon=.false., full_lat=.true., full_lev=.true., async=state%async(async_kmh_lon))
#ifdef Detail_Time
      call Get_End_Time(tran_time_end)
      tran_time = tran_time + tran_time_end - tran_time_start
#endif
      ! call fill_halo_member(block, kmh_lon, full_lon=.false., full_lat=.true., full_lev=.true.)
      ...
      call Get_Start_Time(tran_time_start)
#endif
      call fill_halo_member(block, kmh_lat, full_lon=.true., full_lat=.false., full_lev=.true., async=state%async(async_kmh_lat))
      ! call fill_halo_member(block, kmh_lat, full_lon=.true., full_lat=.false., full_lev=.true.)

      call state%async(async_kmh)%wait()
      call state%async(async_tension_h)%wait()
      call state%async(async_kmh_vtx)%wait()
      call state%async(async_shear_h)%wait()
#ifdef Detail_Time
      call Get_End_Time(tran_time_end)
      tran_time = tran_time + tran_time_end - tran_time_start
      ...
      call state%async(async_kmh_lon)%wait()
      call state%async(async_kmh_lat)%wait()
#ifdef Detail_Time
      call Get_End_Time(tran_time_end)
      tran_time = tran_time + tran_time_end - tran_time_start
#endif
