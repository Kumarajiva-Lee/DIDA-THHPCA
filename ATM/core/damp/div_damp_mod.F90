...
contains
  ...
  subroutine div_damp_run(block, dt, state)
    ...
    call Get_Start_Time(tran_time_start)
      ...
      if (use_implicit_solver(mesh%full_lat_ibeg_no_pole) .eqv. .true. .or. &
          use_implicit_solver(mesh%full_lat_iend_no_pole) .eqv. .true.) call state%async(async_v)%wait() ! 如果是隐式耗散需要等待用新的v
