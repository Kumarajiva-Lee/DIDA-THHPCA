...
contains
  ...
  subroutine read_obs(rc, ntimes, lats_grid, late_grid, lons_grid, lone_grid, obsinfo)
    ...
    do i = 1, obsinfo%obs_nvar
      ...
      do j = 1, obs_num
        val(i, j) = val_buf(i*2-1, j) + randn_1d(j) * sqrt(val_buf(i*2, j))
      enddo
    enddo
    ...
    if (trim(adjustl(obstypename)) == 'GPSRO' ) then
      do i = 1, obs_num
        ...
        do n = 1, obsinfo%obs_nvar
          ...
          obsinfo%obs_rinv(n,lev_buf(i), indx) = 1.0 / val_buf(n*2, i)
        enddo
      enddo
    elseif (trim(adjustl(obstypename)) == 'ADPUPA') then
      do i = 1, obs_num
      ...
      do i = 1, obs_num     
        ...
        do n = 1, obsinfo%obs_nvar
          ...
          obsinfo%obs_rinv(n,1,i) = 1.0 / val_buf(n*2, i)
        enddo
      enddo
    endif
    ...
    do n = 1, obsinfo%obs_nvar
      do i = 1, obsinfo%obs_nz
        do j = 1, obsinfo%obs_num
          obsinfo%obs_rinv(n, i, j) = 1.0 / obs_r_tmp(n, i)
        enddo
      enddo
    enddo
