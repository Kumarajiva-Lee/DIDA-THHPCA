...
contains
  ...
  subroutine latlon_interp_bilinear_cell(src_lon, src_lat, src_data, dst_mesh, dst_data, extrap, zero_pole, ierr)
    ...
    real(r8), allocatable :: data_sum(:,:)
    real(r8) tmp1, tmp2, pole(member_num)
    logical is_found
    ...
    allocate(data_sum(member_num,dst_mesh%full_lon_ibeg:dst_mesh%full_lon_iend))
    ...
    if (dst_mesh%has_south_pole()) then
      if (merge(zero_pole, .false., present(zero_pole))) then
        ...
        data_sum = 0.0_r8
        data_sum(1,:) = dst_data(dst_mesh%full_lon_ibeg:dst_mesh%full_lon_iend,dst_mesh%full_lat_ibeg)
        call zonal_sum_member(proc%zonal_circle, data_sum(:,:), pole(:))
        dst_data(:,dst_mesh%full_lat_ibeg) = pole(1) / global_mesh%num_full_lon
      end if
    end if
    if (dst_mesh%has_north_pole()) then
      if (merge(zero_pole, .false., present(zero_pole))) then
        ...
        data_sum = 0.0_r8
        data_sum(1,:) = dst_data(dst_mesh%full_lon_ibeg:dst_mesh%full_lon_iend,dst_mesh%full_lat_iend)
        call zonal_sum_member(proc%zonal_circle, data_sum, pole)
        dst_data(:,dst_mesh%full_lat_iend) = pole(1) / global_mesh%num_full_lon
      end if
    end if
  ...
  subroutine latlon_interp_bilinear_lon_edge(src_lon, src_lat, src_data, dst_mesh, dst_data, extrap, zero_pole, ierr)
    ...
    allocate(data_sum(member_num,dst_mesh%half_lon_ibeg:dst_mesh%half_lon_iend))
    ...
    if (dst_mesh%has_south_pole()) then
      if (merge(zero_pole, .false., present(zero_pole))) then
        ...
        data_sum = 0.0_r8
        data_sum(1,:) = dst_data(dst_mesh%half_lon_ibeg:dst_mesh%half_lon_iend,dst_mesh%full_lat_ibeg)
        call zonal_sum_member(proc%zonal_circle, data_sum , pole)
        dst_data(:,dst_mesh%full_lat_ibeg) = pole(1) / global_mesh%num_half_lon
      end if
    end if
    if (dst_mesh%has_north_pole()) then
      if (merge(zero_pole, .false., present(zero_pole))) then
        ...
        data_sum = 0.0_r8
        data_sum(1,:) = dst_data(dst_mesh%half_lon_ibeg:dst_mesh%half_lon_iend,dst_mesh%full_lat_iend)
        call zonal_sum_member(proc%zonal_circle, data_sum, pole)
        dst_data(:,dst_mesh%full_lat_iend) = pole(1) / global_mesh%num_half_lon
      end if
    end if
