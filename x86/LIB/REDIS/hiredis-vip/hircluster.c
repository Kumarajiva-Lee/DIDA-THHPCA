...
redisContext *ctx_get_by_node(redisClusterContext *cc, cluster_node *node)
{
    ...
    if(c != NULL)
    {
        if(c->err)
        {
	    logger_log("ctx_get_by_node", "context error, so need reconnect");
            redisReconnect(c);
    ...
    {
	logger_log("ctx_get_by_node", "begin connection");
        c = redisConnect(node->host, node->port);
	logger_log("ctx_get_by_node", "end connection");
    }
...
int __redisClusterGetReplyForTest(redisClusterContext *cc, int slot_num, void **reply)
{
    cluster_node *node;
    redisContext *c;
...
ask_retry:
    ...
    logger_log("sending command", "end");
    return reply;
}
...
int redisClusterAppendFormattedCommand(redisClusterContext *cc, 
    ...
    command->cmd = cmd;
    command->cmd_backup = sdsnewlen(cmd, len);
    command->clen = len;
...
int redisClusterSendAllRobust(redisClusterContext *cc)
{
    dictIterator *di;
    dictEntry *de;
    struct cluster_node *node;
...
int redisClusterGetReplyRetry(redisClusterContext *cc, int slot_num, void *cmd, void **reply) {
    int errno;
    errno = 0;
    struct cmd *command = (struct cmd *)cmd;

    cluster_node *node = node_get_by_table(cc, (uint32_t)slot_num);
    ...
    if (__redisAppendCommand(c, command->cmd_backup, command->clen) != REDIS_OK) 
    {
        __redisClusterSetError(cc, c->err, c->errstr);
        return REDIS_ERR;
    }
...
int redisClusterGetReply(redisClusterContext *cc, void **reply) {
    ...
    *reply = NULL;
    ...
    {
        errno = 0;
        //printf("getreply from slot %d; command is %s\n\n", slot_num, command->cmd);
        errno = __redisClusterGetReply(cc, slot_num, reply);
        if (command->cmd_backup != NULL) {
            //printf("cmd_backup is %s\n", command->cmd_backup);
            /* read timeout */
            if (errno == REDIS_ERR && cc->err == REDIS_ERR_TIMEOUT) {
                //printf("cmd_backup is %s\n", command->cmd_backup);
                //printf("begin getReplyRetry\n");
                errno = redisClusterGetReplyRetry(cc, slot_num, command, reply);
            }
        } 
    ...
    if(commands == NULL)
    {
        printf("subcommands is not null\n\n", slot_num, command->cmd);
        __redisClusterSetError(cc,REDIS_ERR_OTHER,
