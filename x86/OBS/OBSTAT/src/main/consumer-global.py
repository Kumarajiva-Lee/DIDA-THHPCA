...
if obs_open == 1:
    ...
    NumYGrid_obs=3 #conn.hlen(obs_hashkey_lat)
    NumXGrid_obs=5 #conn.hlen(obs_hashkey_lon)
    ...
    for time_in in range(0,Numtime):
        ...
        for vari_in in range(nl_start, nl_end):
            ...
            if variable_in_key == 'ps':
                level_in_range = len(vari_surf_lev_id)
            else:
            ...
            for level_in in range(0,level_in_range):
                ...
                #key_get_4 = np.array(key_get_4).flatten().tolist()
                ...
                #print('key_get_4',key_get_4)
                #print('key_get_obs',key_get_obs)
                ...
                tr = tr+list(tr_obs_1lev)
                bg = bg+list(bg_obs_1lev)
                an = an+list(an_obs_1lev)
                ob = ob+list(obs_1lev)
        ...
        tr_global_array = np.array(tr_get_global).reshape(level_in_range,NumYGrid,NumXGrid)
        amb_global_array = np.array(amb_get_global).reshape(level_in_range,NumYGrid,NumXGrid)
        bg_global_array = np.array(bg_get_global).reshape(level_in_range,NumYGrid,NumXGrid)
        ...
        grmse = indicator.cal_rmse(an_global_array, tr_global_array)

        for level_in in range(0,level_in_range):
            an_global_array[level_in]=an_global_array[level_in]*lat_theta

        grmse_theta = indicator.cal_rmse(an_global_array, tr_global_array)
...
#--------------------------------------------------no obs----------------------------------------------------------------------
    ...
    for time_in in range(0,Numtime):
        ...
        #print('an_global_array',an_global_array[0,0])
        #print('tr_global_array',tr_global_array[0,0])

        grmse = indicator.cal_rmse(an_global_array, tr_global_array)
        print(grmse)

        for level_in in range(0,level_in_range):
            tr_global_array[level_in]=tr_global_array[level_in]*lat_theta
            an_global_array[level_in]=an_global_array[level_in]*lat_theta
