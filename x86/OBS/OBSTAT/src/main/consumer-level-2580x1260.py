...
NumXGrid = 2580 #int(conn.hget(casename,'num_lon')) #360
...
hashkey_tr = 'yuz:'+res_in_key #'realfield:1deg-201902'
...
lon_get = np.arange(0,360,360/2580)
    ...
    for proc_in in range(nl_start, nl_end):
        ...
        #for lat_in in range(1,5+1):
            ...
            #for lon_in in range(1,5+1): 
                ...
                pipe1.hmget(hashkey_tr+':'+str(time_in_key)+":"+str(lon_in_key), key_str)
                pipe2.hmget(hashkey_bg+':'+str(time_in_key)+":"+str(lon_in_key), key_str)
                pipe3.hmget(hashkey_amb+':'+str(time_in_key)+":"+str(lon_in_key), key_str)
                #print(key_get[2])
        ...
        t2=time.time()
        print("hmget：",t2-t1)
        print("len_get:", len(tr_get))

        #print(amb_get)

        tr_get = [float(x[0]) for x in tr_get]
        bg_get = [float(x[0]) for x in bg_get]
        amb_get = [float(x[0]) for x in amb_get]
