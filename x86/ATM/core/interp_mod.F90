...
contains
  ...
  stencil_time(3) = stencil_time(3) + stencil_time_end - stencil_time_start
#endif
  ...
  subroutine interp_lev_edge_to_cell(mesh, x_lev, x, w, upwind_wgt_)
    ...
    real(r8), intent(inout) :: x(member_num ,                       &
                                 ...
                                 mesh%full_lat_lb:mesh%full_lat_ub, &
                                 mesh%full_lev_lb:mesh%full_lev_ub)
    real(r8), intent(in), optional :: w(member_num ,&
                                        mesh%full_lon_lb:mesh%full_lon_ub, &
                                        mesh%full_lat_lb:mesh%full_lat_ub, &
                                        mesh%full_lev_lb:mesh%full_lev_ub)
    real(r8), intent(in), optional :: upwind_wgt_
      ...
      case (1)
        do k = mesh%full_lev_ibeg + 1, mesh%full_lev_iend - 1
          do j = mesh%full_lat_ibeg, mesh%full_lat_iend
            do i = mesh%full_lon_ibeg, mesh%full_lon_iend
              do im = 1 , member_num
                x(im,i,j,k) = upwind1(sign(1.0_r8, w(im,i,j,k)), beta, x_lev(im,i,j,k:k+1))
              end do  
            end do
          end do
        end do
      ...
      case (3)
        do k = mesh%full_lev_ibeg + 1, mesh%full_lev_iend - 1
          do j = mesh%full_lat_ibeg, mesh%full_lat_iend
            do i = mesh%full_lon_ibeg, mesh%full_lon_iend
              do im = 1 , member_num
                x(im,i,j,k) = upwind3(sign(1.0_r8, w(im,i,j,k)), beta, x_lev(im,i,j,k-1:k+2))
              end do
            end do
          end do
        end do
  ...
  subroutine interp_cell_to_vtx(mesh, x, x_vtx, debug)
    ...
    debug_flag = merge(debug , .false. , present(debug))
      ...
      case (3)
        do k = mesh%half_lev_ibeg + 2, mesh%half_lev_iend - 2
          do j = mesh%full_lat_ibeg, mesh%full_lat_iend
            do i = mesh%full_lon_ibeg, mesh%full_lon_iend
              do im = 1 , member_num
                x_lev(im,i,j,k) = weno3(sign(1.0_r8, w(im,i,j,k)), x(im,i,j,k-2:k+1))
              end do
            end do
          end do
        end do
      ...
      case (1)
        do k = mesh%half_lev_ibeg + 1, mesh%half_lev_iend - 1
          do j = mesh%full_lat_ibeg, mesh%full_lat_iend
            do i = mesh%full_lon_ibeg, mesh%full_lon_iend
              do im = 1 , member_num
                x_lev(im,i,j,k) = upwind1(sign(1.0_r8, w(im,i,j,k)), beta, x(im,i,j,k-1:k))
              end do
            end do
          end do
        end do
      ...
      case (3)
        do k = mesh%half_lev_ibeg + 2, mesh%half_lev_iend - 2
          do j = mesh%full_lat_ibeg, mesh%full_lat_iend
            do i = mesh%full_lon_ibeg, mesh%full_lon_iend
              do im = 1 , member_num
                x_lev(im,i,j,k) = upwind3(sign(1.0_r8, w(im,i,j,k)), beta, x(im,i,j,k-2:k+1))
              end do
            end do
          end do
        end do
        ...
        do i = mesh%full_lon_ibeg, mesh%full_lon_iend
          do k = mesh%full_lev_iend, mesh%full_lev_ibeg + 1, -1
            if (z(im,i,j,k) <= zo .and. zo <= z(im,i,j,k-1)) then
              ...
              exit
            else if (zo < z(im,i,j,k) .and. k == mesh%full_lev_iend) then
              z1 = z(im,i,j,k  ) - zo
              z2 = z(im,i,j,k-1) - zo
              a  =  z2 / (z2 - z1)
              b  = -z1 / (z2 - z1)
              ...
              exit
            else if (zo > z(im,i,j,k-1) .and. k == mesh%full_lev_ibeg + 1) then
              z1 = zo - z(im,i,j,k-1)
              z2 = zo - z(im,i,j,k  )
              a  =  z2 / (z2 - z1)
              b  = -z1 / (z2 - z1)
