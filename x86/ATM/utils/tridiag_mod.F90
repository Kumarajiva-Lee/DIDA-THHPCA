...
contains
  ...
  subroutine tridiag_solver_clone(this, other)
    ...
    case default
      if (is_root_proc()) call log_error('Clone: Uninitialized tridiag solver!', __FILE__, __LINE__)
    end select

  end subroutine tridiag_solver_clone

  subroutine tridiag_solver_solve(this, rhs, x, member_id)

    class(tridiag_solver_type), intent(inout) :: this
    real(r8), intent(in) :: rhs(:,:)
    real(r8), intent(out) :: x(:,:)
    integer , intent(in) , optional :: member_id

    select case (this%solver)
#ifdef HAS_MKL
    case (1)
      call this%mkl_solver%solve(rhs(member_id,:), x(member_id,:))
#endif
    case (2)
      call this%spk_solver%solve(rhs, x)
    case default
      if (is_root_proc()) call log_error('Solve: Uninitialized tridiag solver!', __FILE__, __LINE__)
    end select
